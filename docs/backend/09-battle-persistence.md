...
Read by the game through `GET /api/v1/battle-missions`; authored by the Crystalline Rift Studio through the
...
domain, generated by Crystalline Rift Studio's *Export seed migration*) re-seeds all ten by authored id with
...
Damage tuning lives in a dual-DB (SQLite + Postgres) `damage_curve` table, authored as `DamageCurveDefinition` ScriptableObjects in Unity and synced from the Crystalline Rift Studio **Battle Tuning** tab.
...
`BattleDomainService` syncs this list to `generated_creature_status_conditions` on every resolved action: a condition absent from both the resolver's remaining and applied lists is removed via `RemoveStatusConditionAsync`; remaining conditions are re-applied (updating `turns_remaining`) via `ApplyStatusConditionAsync`. `BuildSnapshotAsync` returns the snapshot together with a `ConditionsKnown` flag — false when the condition-definition load threw and the resolver was handed an empty condition list — and the reconcile step is skipped entirely in that case. Without the flag, that swallowed failure read as "every condition just expired" and stripped a status the opponent had just spent a turn landing; with it, the reconcile still reaps every genuinely expired row when the load succeeds, including one whose definition has since been soft-deleted.
...
set as a pure function, so the endpoint and the Crystalline Rift Studio's inline form cannot disagree about
...
to `[0, 10]`, in the endpoint, in `BattleConfigurationDomainService`, and in the Crystalline Rift Studio's grid.
...
token the Crystalline Rift Studio gets from `POST /auth/service-token`. A player-scoped token gets `403`;
