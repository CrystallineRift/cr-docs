...
`BattleDomainService` syncs this list to `generated_creature_status_conditions` on every resolved action: only a condition the pre-action snapshot actually carried (`CreatureSnapshot.ActiveConditions`) is a candidate for removal, and one of those absent from both the resolver's remaining and applied lists is removed via `RemoveStatusConditionAsync`; remaining conditions are re-applied (updating `turns_remaining`) via `ApplyStatusConditionAsync`. The snapshot restriction matters because `BuildSnapshotAsync` swallows a failed condition-definition load and hands the resolver an empty condition list — without it, that swallowed failure read as "every condition just expired" and stripped a status the opponent had just spent a turn landing.
