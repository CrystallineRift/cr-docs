...
**Tab layout (23 tabs, grouped in the nav rail):**
...
| 22 | CONTENT | Dialogues | `AssetDatabase.FindAssets("t:DialogueDefinition")` | `DialogueEditorSyncHelper.Sync` |
...
| 21 | COMBAT | Battle Reactions | `AssetDatabase.FindAssets("t:CreatureReactionProfile")` | none — local assets, never pushed |
...
- **Battle Reactions tab (21)** — `CreatureReactionProfile` assets (`Assets/CR/Content/Defs/ReactionProfiles/`): how a creature's own body behaves in battle. Local-only — the presenter reads them through `CreatureDefinition.reactionProfile`, nothing is pushed, so the action row has search and **+ New Reaction Profile** and no sync pair (`DrawLocalSoActionRow`). Each row is the profile's display name over a digest (`ReactionProfileRow.Describe`: *Inherits Standard Reactions · 15 of 15 beats covered · 3 authored here · used by 2 creatures*, from `CreatureReactionProfileBeats.Coverage` and a count over the registry's creatures); **Variant** creates a profile inheriting from that row and selects it; clicking the row opens the profile's own `CreatureReactionProfileEditor` inline — the same beat list, coverage bar and override/clear controls as the Inspector, which is the point: authoring a profile no longer needs an Inspector window per asset. `CreatureReactionProfileEditor.Reveal` routes "Create variant…", and the creature inspector's **Edit reactions**, through `ContentStudioTool.NavigateTo` when drawn inside the Studio (`IsShowingInline`), and to the Inspector otherwise.
