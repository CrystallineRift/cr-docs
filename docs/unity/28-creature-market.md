...
| `SellEligibility` | `Evaluate(isTradable, isOnTeam, teamCount, activeListings, maxListings, askingPrice, listingFee, currentGold)`: tradable → last-team-member → listing cap (`activeListings >= maxListings`) → price>0 → fee affordable, in the same order `MarketService.ListCreatureAsync` checks server-side. A team creature is refused only when it is the trainer's *only* remaining team member (`isOnTeam && teamCount <= 1`); otherwise it may be listed like any storage creature. The cap is per **account**, not per creature — `activeListings` is the account's current count of Active listings across every creature, and refuses with `"You've reached your listing limit ({maxListings})."` before the price/afford checks run. |
...
**`PaymentRequiredException`** (`Assets/CR/Core/Data/Client/Implementation/PaymentRequiredException.cs`)
